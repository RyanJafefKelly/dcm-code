...
(a, kappa, q_j) and evaluates the PPC at Rater_B's focus cells (Human / ELIZA)
and at Rater_B × LLMs under several fixed values of sigma_Derek. Other experts
remain at sigma = 1.
...
def main() -> None:
    ...
    print("Running sigma_Derek sweep across Rater_B's three systems...")
    ...
    lines.append(
        ...
        "and vary only sigma_Derek -- the noise scale of Rater_B's ordered-probit "
    ...
    lines.append(
        "**Goal:** verify that sigma_Derek < 1 moves Rater_B x Human cat-7 and "
        "Rater_B x ELIZA cat-1 PPCs toward observed, without degrading Rater_B x "
    ...
    # Per-sigma rating histogram for Rater_B x LLMs
    llm_rows = all_results.get("2024 Leading Chat LLMs", [])
    if llm_rows:
        lines.append("\n## Rater_B × LLMs rating histograms (per sigma_D)\n")
        lines.append(
    ...
    for sys_name in DEREK_SYSTEMS:
        ...
        lines.append(
            f"- **Rater_B × {SYSTEM_DISPLAY.get(sys_name, sys_name)}**: "
    ...
    if consistent:
        ...
        lines.append(
            f"All three Rater_B cells prefer sigma_D ≈ {s}. That is strong "
            "evidence the sharpness mechanism is consistent across Rater_B's "
        ...
        lines.append(
            "Best sigma differs across Rater_B's three cells. That means a single "
            "sigma_D parameter cannot fully reconcile Rater_B's Human / ELIZA "
