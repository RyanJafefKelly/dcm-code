...
is 1 for all experts except Rater_B (who gets the candidate sigma_D), and the
product runs over *all* ratings of indicator j (not just Rater_B's). The predictive
mixture for Rater_B's rating of indicator j is then
...
def run_corrected_sweep(
    ...
    # Per-indicator: collect full rating list + tree-implied q draws + flag Rater_B's rating
    ...
    for sigma_D in sigma_grid:
        ...
        # Aggregate predictive histogram across all of Rater_B's ratings
        ...
        for ind in indicator_rows:
            ...
            # Rater_B's predictive category distribution per draw
            # P(r=k | Rater_B, indicator j, draw s, sigma_D)
            ...
            # Aggregate over draws (mean) for this indicator, then aggregate across Rater_B's
...
def main() -> None:
    ...
    print("Running corrected sweep across Rater_B's three systems...")
    ...
    lines.append(
        "Rater_B's σ varies; all other experts held at σ=1. $(a, \\kappa, q_j)$ frozen "
    ...
    lines.append(
        "Under the corrected sweep, smaller σ_Derek sharpens Rater_B's emissions AND "
