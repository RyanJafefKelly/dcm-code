...
(rating-SDT) model for 7-point ordinal expert responses. The paper tree is used
as a marginal probability propagation device; indicator leaves are conditionally
independent given the propagated q_j values and shared parameters.
...
class ModelConfig:
    ...
    # Concentration for every paper-tree node's Beta(alpha, beta).
    ...
    def __post_init__(self) -> None:
        """Validate baseline ordinal configuration."""
        if self.INDICATOR_STATE_MODEL not in {"binary", "three_state"}:
            raise ValueError(
                "INDICATOR_STATE_MODEL must be 'binary' or 'three_state'"
            )
...
def anonymized_expert_labels(expert_names: List[str]) -> Dict[str, str]:
    """Map raw expert identifiers to stable public labels."""
    return {name: f"Expert {_alpha_label(i)}" for i, name in enumerate(expert_names)}
...
class OrdinalDataProcessor:
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.expert_display_names: Dict[str, str] = {}
    ...
    def process(self, stance_data: Dict, system: str) -> "OrdinalDataProcessor":
        ...
        self.expert_display_names = anonymized_expert_labels(self.expert_names)
        self.logger.info(
            "Anchor expert: %s (%s observations)",
            self.expert_label(self.anchor_expert),
            expert_counts[self.anchor_expert],
        )
        self.logger.info(
            "Expert mapping: %s",
            {self.expert_display_names[name]: idx for name, idx in self.expert_to_idx.items()},
        )
    ...
    def expert_label(self, expert: Any) -> str:
        """Return the public anonymized label for an expert."""
        if expert is None:
            return "Expert ?"
        if isinstance(expert, int):
            expert = self.expert_names[expert]
        return self.expert_display_names.get(expert, "Expert ?")
...
class EvidenceProcessor:
    ...
    def get_beta_parameters(
        ...
        """Paper-tree Beta parameters for parent-present and parent-absent states.
...
class BayesianModelBuilder:
    ...
    def _create_node_variable(
        ...
        This is the leaf-independent marginal-tree DGP, not the exact latent
        tree with shared internal states.
        ...
        if varname:
            if node_type == "indicator":
                ...
                if pz1 is not None:
                    ...
                    expected_z = self._get_var_mean(idata, f"{varname}_expected_z")
                    if expected_z is not None:
                        print(
                            f"{prefix}  E(indicator strength | data): "
                            f"{expected_z:.4f}"
                        )
                    else:
...
def fetch_data(config: ModelConfig) -> List[Dict]:
    """Fetch DCM data from the configured API endpoint."""
    import requests

    response = requests.get(config.API_ENDPOINT, timeout=60)
    response.raise_for_status()
    return response.json()


def load_data(config: ModelConfig, fetch_if_missing: bool = False) -> List[Dict]:
    """Load DCM data from local cache, optionally regenerating it from the API."""
    path = Path(config.DATA_CACHE_PATH)
    if not path.exists():
        if not fetch_if_missing:
            raise FileNotFoundError(
                f"Data cache not found at {path}. "
                f"Place data_cache.json in the working directory or call "
                "load_data(..., fetch_if_missing=True)."
            )
...
def main() -> None:
    ...
    if "b_free" in idata.posterior.data_vars:
        ...
        print(
            f"  b (expert shifts):  "
            f"[0.000 (anchor: {processor.expert_label(processor.anchor_expert)}), "
...
class MultiSystemDataProcessor:
    ...
    def process(
        ...
        self.expert_display_names = anonymized_expert_labels(self.expert_names)
        self.logger.info(
            "Merged expert pool (%s): %s anchor=%s",
            len(self.expert_names),
            [self.expert_display_names[name] for name in self.expert_names],
            self.expert_label(self.anchor_expert),
        )
