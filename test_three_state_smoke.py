...
def _ppc_checks(
    ...
    *,
    ...
    # inspect_pz1_for_cell dispatches on state model
    expert_names = processor.expert_names
    e_name = expert_names[0]
    rows = inspect_pz1_for_cell(idata, builder, processor, e_name, sys_a)
    assert rows, (
        f"inspect_pz1_for_cell ({state_model}) returned no rows for "
        f"{e_name} x {sys_a}"
    )
    for row in rows:
    ...
    print(
        f"  inspect_pz1_for_cell ({state_model}) OK: {len(rows)} "
